//! The modules for storing client data on the file system and accessing it
...
impl SqrlData {
    ...
    /// ```rust
    /// use sqrl_client::SqrlData;
    ///
    /// let (mut client, _) = SqrlData::new("password").unwrap();
    ...
    /// Load SqrlData from file
    pub fn from_file(file_path: &str) -> Result<Self> {
        SqrlData::from_binary(convert_vec(std::fs::read(file_path)?))
    }

    /// Save SqrlData to file
    ...
    /// Generate SqrlData from base64 encoded data
    pub fn from_base64(input: &str) -> Result<Self> {
        ...
        SqrlData::from_binary(binary)
    }

    /// Convert SqrlData to base64 encoding
    pub fn to_base64(&self) -> Result<String> {
        let data = self.to_binary()?;
        Ok(BASE64_URL_SAFE.encode(data))
    }

    /// Take textual identity format and generate SqrlData from it
    pub fn from_textual_identity_format(
        ...
        SqrlData::from_identity_unlock(identity_unlock, rescue_code, new_password)
    }
...
impl SqrlDataProvider for SqrlData {
    fn get_signing_key(
        &self,
        password: &str,
        url: &str,
        ...
        key.get_private_key(url, alternate_identity)
    }
    ...
    fn get_previous_signing_keys(
        ...
        if let Some(previous_identity_data) = &self.previous_identities {
            let identity_master_key = self
                .user_configuration
                .decrypt_identity_master_key(password)?;

            if let Some(previous_identities) =
                previous_identity_data.get_previous_identities(&identity_master_key)?
            {
                let mut result: VecDeque<SigningKey> = VecDeque::new();
                for identity in previous_identities {
                    match identity.get_private_key(url, alternate_identity) {
                        Ok(signing_key) => result.push_back(signing_key),
                        Err(error) => {
                            return Err(SqrlError::new(format!(
                                "Failed to get signing key from previous identity: {}",
                                error
                            )))
                        }
                    }
    ...
    fn get_secret_index_key(
        ...
        let private_key = self.get_signing_key(password, url, alternate_identity)?;
    ...
    ///
    /// let (mut client, _) = SqrlData::new("password").unwrap();
    /// client.verify_password("password").unwrap();
    /// ```
...
impl GetKey for IdentityKey {
    fn get_private_key(&self, url: &str, alternate_identity: Option<&str>) -> Result<SigningKey> {
        ...
        let data = match alternate_identity {
            Some(id) => {
                holder = format!("{}{}", sqrl_url.get_auth_domain(), id);
                &holder
            }
            None => &sqrl_url.get_auth_domain(),
        };
...
fn encode_textual_identity(client: &SqrlData) -> Result<String> {
...
mod tests {
    ...
    #[test]
    fn load_test_data() {
        let client = SqrlData::from_file(TEST_FILE_PATH).unwrap();
    ...
    fn try_textual_identity_loading() {
        let client = SqrlData::from_textual_identity_format(
