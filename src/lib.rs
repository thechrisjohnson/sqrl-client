    ...
    DataProvider: SqrlDataProvider,
{
    ...
    pub async fn authenticate(
        ...
        let mut known_signing_key = self.get_known_signing_key(password, url, alternate_identity).await?;
    ...
    pub async fn get_known_signing_key(
        ...
        let previous_keys = match self.sqrl_data_provider.get_previous_signing_keys(password, url, alternate_identity)? {
            Some(keys) => keys,
            None => VecDeque::new()
        };

        let mut previous_signing_key = previous_keys.pop_front();
        let client_request = ClientRequest::new(
            query_client_params,
            query_server_data,
        ...
        loop {
            ...
            {
                return Ok(Some(current_signing_key));
            } else if server_response
                .transaction_indication_flags
                .contains(&TIFValue::PreviousIdMatch)
            {
                return Ok(previous_signing_key);
            }
